...
class _ProfissionalAgendaPageState extends State<ProfissionalAgendaPage> {
  ...
  bool _isGridView = true; // Iniciamos em modo grid por padrão conforme solicitado
    ...
    try {
      ...
      if (mounted) {
        setState(() {
          ...
          _blockedDays = monthlyBlocks.map((b) => DateTime.parse(b['data'])).toList();
        });
      }
    } catch (e) {
  ...
  Future<void> _loadAppointments() async {
    ...
    try {
      ...
      final blocksData = await _profRepo.getProfessionalBlocksAndLunch(professionalId, _selectedDate);
      ...
      final startRange = _isMonthlyView
          ...
          : DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day).toIso8601String();
      final endRange = _isMonthlyView
          ? DateTime(_selectedDate.year, _selectedDate.month + 1, 0, 23, 59, 59).toIso8601String()
          : DateTime(_selectedDate.year, _selectedDate.month, _selectedDate.day, 23, 59, 59).toIso8601String();

      var query = _supabase
          .from('agendamentos')
          .select('*, servicos(nome, duracao_minutos), perfis!cliente_id(id, nome_completo, avatar_url), profissional:perfis!profissional_id(nome_completo)')
  ...
  Future<void> _updateStatus(Map<String, dynamic> appointment, String newStatus) async {
    ...
    if (newStatus == 'concluido') {
      ...
      if (apptDate.isAfter(now)) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: const Text('Não é possível finalizar um agendamento futuro.'), backgroundColor: AppColors.warning),
        );
        return;
      }
    }
    ...
    try {
      ...
      if (newStatus == 'pagar' || (newStatus == 'concluido' && paymentData.isNotEmpty)) {
        final profResponse = await _supabase.from('perfis').select('comissao_agendamentos_percentual').eq('id', profissionalId).single();
      ...
      if (newStatus == 'pagar') {
        ...
        updates['pago'] = true;
        if (appointment['pago'] != true) updates['data_pagamento'] = DateTime.now().toUtc().toIso8601String();
      } else {
      ...
      await _supabase.from('agendamentos').update(updates).eq('id', appointmentId);
      ...
      String message = 'O status do seu agendamento de $serviceName foi atualizado.';
      ...
      switch(newStatus) {
        case 'concluido': 
          ...
          message = 'Seu atendimento de $serviceName em $dateStr às $timeStr foi finalizado. Esperamos que tenha gostado!';
      ...
      await _notificationRepo.saveNotification(NotificationModel(
        ...
        mensagem: 'Você alterou o status do agendamento de $serviceName ($clientName) para ${newStatus.toUpperCase()}.',
        tipo: 'logger',
        isLida: false,
        dataCriacao: DateTime.now(),
      ));
  ...
  Future<void> _marcarComoNaoPago(Map<String, dynamic> appointment) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Agendamento marcado como NÃO PAGO'), backgroundColor: AppColors.error),
        );
      }
    } catch (e) {
    ...
    setState(() {
      ...
      if (offset > 0) {
        ...
        if (_selectedDate.day == lastDayOfMonth) {
          ...
          final newDay = _selectedDate.day > lastDayNextMonth ? lastDayNextMonth : _selectedDate.day;
        ...
        if (_selectedDate.day == 1) {
          ...
          final newDay = _selectedDate.day > lastDayPrevMonth ? lastDayPrevMonth : _selectedDate.day;
                ...
                child: Container(
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color: isSelected 
                          ? Colors.transparent 
                          : (isUnavailable ? AppColors.error.withOpacity(0.2) : primaryGreen.withOpacity(0.05)),
                    ),
                  ),
                  child: Center(
                    child: Column(
                      ...
                      children: [
                        Text(
                          ...
                          style: TextStyle(fontSize: 14,
                            ...
                            color: isSelected 
                                ? AppColors.white 
                                : (isUnavailable ? AppColors.error.withOpacity(0.5) : AppColors.textPrimary),
                          ),
                        ),
                        if (isUnavailable && !isSelected)
                          Icon(Icons.lock_clock, size: 8, color: AppColors.error.withOpacity(0.5)),
                      ],
                    ),
                  ),
                ),
          ...
          child: Column(
            ...
            children: [
              Text(
                ...
                style: TextStyle(fontSize: 10,
                  fontWeight: isSelected ? FontWeight.bold : FontWeight.normal,
                  color: isSelected ? AppColors.white.withOpacity(0.7) : (isUnavailable ? AppColors.error.withOpacity(0.4) : AppColors.textLight),
                ),
              ),
              ...
              Text(
                ...
                style: TextStyle(fontSize: 18,
                  fontWeight: FontWeight.bold,
                  color: isSelected ? AppColors.white : (isUnavailable ? AppColors.error.withOpacity(0.5) : AppColors.textPrimary),
                ),
              ),
              if (isUnavailable && !isSelected)
                Padding(
                  padding: const EdgeInsets.only(top: 2),
                  child: Icon(Icons.lock_clock, size: 10, color: AppColors.error.withOpacity(0.5)),
                ),
            ],
          ),
        ),
                ...
                builder: (context, constraints) {
                  return Stack(
                    children: [
                      ...
                      // Almoço (se houver)
                      if (_lunchTime != null) _buildLunchBlock(hourHeight, startHour, constraints.maxWidth),

                      // Bloqueios de Agenda
                      ..._buildAgendaBlockCards(hourHeight, startHour, constraints.maxWidth),
  ...
  Widget _buildLunchBlock(double hourHeight, int startHour, double maxWidth) {
    ...
    final durationHours = (fim.hour + fim.minute / 60.0) - (inicio.hour + inicio.minute / 60.0);
  ...
  List<Widget> _buildAgendaBlockCards(double hourHeight, int startHour, double maxWidth) {
    return _agendaBlocks.map((block) {
      ...
      if (!isDiaTodo && block['hora_inicio'] != null && block['hora_fim'] != null) {
  ...
  List<Widget> _buildContinuousAppointmentCards(double hourHeight, int startHour, double maxWidth) {
    ...
    if (sortedAppts.isNotEmpty) {
      ...
      DateTime clusterEnd = _getEnd(sortedAppts[0]).add(const Duration(minutes: 5)); // Pequeno buffer de segurança
    ...
    for (final cluster in clusters) {
      ...
      for (final appt in cluster) {
        ...
        for (int i = 0; i < columns.length; i++) {
          ...
          for (final other in columns[i]) {
            ...
            if (effectiveStart.isBefore(effectiveOtherEnd) && end.isAfter(otherStart)) {
              overlaps = true;
              break;
            }
          }
      ...
      final double clusterColWidth = math.max(100.0, (maxWidth - 10) / totalColsInCluster);
  ...
  Widget _buildAppointmentCard({
    ...
    if (status == 'concluido' || status == 'confirmado') statusColor = AppColors.success;
    ...
    showModalBottomSheet(
      ...
      builder: (context) => AdminReagendamentoModal(
        ...
        onReagendado: () {
          _loadAppointments();
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Agendamento alterado com sucesso!'), backgroundColor: AppColors.primary),
          );
        },
      ),
    );
  ...
  Future<void> _switchProfessional(Map<String, dynamic> appt) async {
    try {
      ...
      final allProfsForService = await _profRepo.getProfessionalsByService(serviceId);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Profissional alterado com sucesso!'), backgroundColor: AppColors.primary),
        );
        _loadAppointments();
      }
    } catch (e) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Erro ao realizar a troca. Tente novamente.'), backgroundColor: AppColors.error),
        );
      }
    }
  }
    ...
    return SliverList(
      delegate: SliverChildBuilderDelegate(
        (context, index) {
          ...
          final String headerDate = DateFormat("EEEE, dd 'de' MMMM 'de' yyyy", 'pt_BR').format(dateTime);
          return Column(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: [
              Padding(
